...
# GTK3 uses Gtk.TargetEntry for drag and drop targets
...
class MainWindow(object):
    ...
    def __init__(self):
        ...
        #self.symbmapping={"(":")", "{":"}", "[":"]", "'":"'", "\"":"\""} 
        #Widget tree.. (using Gtk.Builder for GTK3)
        self.wTree=gtk.Builder()
        self.wTree.add_from_file(os.path.join(os.path.dirname(__file__), 'ui', 'hello.glade'))
    ...
    def _prepare_toolbar(self):
        ...
        newtbitem=gtk.ToolButton(stock_id="gtk-new")
        newtbitem.connect("clicked", self.on_menuitemnew_activate)
        
        opentbitem=gtk.ToolButton(stock_id="gtk-open")
        opentbitem.connect("clicked", self.on_menuitemopen_activate)
        
        savetbitem=gtk.ToolButton(stock_id="gtk-save")
        savetbitem.connect("clicked", self.on_menuitemsave_activate)
        
        saveastbitem=gtk.ToolButton(stock_id="gtk-save-as")
        saveastbitem.connect("clicked", self.on_menuitemsaveas_activate)
                
        reverttbitem=gtk.ToolButton(stock_id="gtk-revert-to-saved")
        reverttbitem.connect("clicked", self._on_revert)
        
        colorchoosertbitem=gtk.ToolButton(stock_id="gtk-select-color")
    ...
    def _prep_languages(self):
        langsmenu=gtk.Menu()
        adarmitem=gtk.RadioMenuItem(label="ada")
        ...
        for lang in langs_ids:
            if lang=="ada": 
                ...
                mitem=gtk.RadioMenuItem(group=adarmitem, label=lang)
                mitem.connect("toggled", self._on_languagepicked, lang)
                langsmenu.append(mitem)
        # Handle both MenuItem (old) and Action (from converted glade)
        if hasattr(self._menuitemlanguage, 'set_submenu'):
            self._menuitemlanguage.set_submenu(langsmenu)
        else:
            # If it's an Action, we need to find the associated menu item
            # For now, skip setting up the language menu
            pass
    ...
    def on_chkmenuitemdrawspaces_toggled(self, widget, *args):
        # set_draw_spaces deprecated in gtksourceview4
        try:
    ...
    def forkme(self, path=""):
        ...
        sb.Popen(["python3", helloedpath, path])
    ...
    def _init_editor(self):
        ...
        self._sv=sv.View.new_with_buffer(sv.Buffer())
        self.bcvh=BracketCompletionViewHelper(self._sv)
        self._scrolledwindoweditor.add(self._sv)
        #set default font.
        font_desc = pango.FontDescription("Monospace 10")
        if font_desc:
            # Use CSS provider for GTK3 instead of deprecated modify_font
            try:
                self._sv.modify_font(font_desc)
            except AttributeError:
                # GTK3.16+ uses CSS
        ...
        # set_draw_spaces deprecated in gtksourceview4, use set_show_whitespace if available
        try:
        ...
        #Hook it up.
        # size-request signal removed in GTK3, use configure-event if needed
        # self._window1.connect("size-request", self.on_window1_size_request)
        
        
    def on_window1_size_request(self, *args):
        # Deprecated - size-request signal not available in GTK3
        # w, h=self._window1.get_size()
    ...
    def search_text(self, text, backward=False, all=False):
        ...
        if text:
            ...
            if not backward:
                ...
                result = find_iter.forward_search(text, flags=0, limit=None)
                if result is None:
                ...
                result = find_iter.backward_search(text, flags=0, limit=None)
                if result is None:
